...
#PGPASSFILE=/etc/haproxy/pgpass psql "host=$HAPROXY_SERVER_ADDR port=$HAPROXY_SERVER_PORT connect_timeout=1" --username=postgres --no-password --pset=tuples_only -c "SELECT pg_is_in_recovery();" | grep -P "^\s*f\s*$" > /dev/null

# Assumes keeper metric port is postgres port plus 1.
let STKEEPER_METRICS_PORT=$HAPROXY_SERVER_PORT+1
curl http://$HAPROXY_SERVER_ADDR:$STKEEPER_METRICS_PORT/metrics 2>/dev/null | grep -P "^stolon_keeper_local_role.*master.*1$" > /dev/null
